...
/*   Updated: 2023/05/10 06:47:40 by yejinkim         ###   ########seoul.kr  */
/*                                                                            */
/* ************************************************************************** */
...
char	**pars_envp(char **envp)
{
	...
	if (!envp[i])
		print_error(errmsg(TRUE, NULL, NULL, "No such file or directory"), TRUE, 127);
...
void	exec_cmd(int fds[2], t_exec *exec, int flag)
{
	...
	if (execve(pl->path, pl->cmd, pl->env->value) == -1)
		print_error(errmsg(TRUE, pl->cmd[0], NULL, "command not found"), TRUE, 127);
}
